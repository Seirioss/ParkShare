package com.ych.views;

import com.ych.parkshare.R;
...
public class LinkClickTextView extends TextView {
	...
	private boolean handledLinkTouch(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			if (mSelectedLink != null) {
				...
				postDelayed(mUnsetLinkPressedState, ViewConfiguration.getPressedStateDuration());
				mSelectedLink = null;
				return true;
			}
			break;
	...
	private void setLinkPressed(boolean pressed) {
		...
		if (pressed) {
			buffer.setSpan(mForegroundColorSpan, buffer.getSpanStart(mSelectedLink), buffer.getSpanEnd(mSelectedLink), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		} else {
